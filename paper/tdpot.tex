...
\relatedversiondetails[cite=wz-cplttdap-22]{Full version}{http://arxiv.org/abs/2207.00381} %linktext and cite are optional
...
Then, we check if we can improve the lower bound from $v$ to $t$, i.e.\ $\mathtt{D}[v] \leftarrow \min(\mathtt{D}^{\downarrow}[v], \mathtt{D}[u] + d)$.
...
To obtain a distance estimate for vertex $u$, we first recursively compute distance estimates $\mathtt{D}[v]$ for all upward neighbors $v$ where $uv \in \echu$.
Then, we use AILR to obtain an arrival interval $[\tau_{\min}, \tau_{\max}]$ at $u$.
Finally, we relax the upward edges $uv$ set $\mathtt{D}[u] \leftarrow \min(\mathtt{D}^{\downarrow}[u], \mathtt{D}[v] + \min_{\tau \in [\tau_{\min}, \tau_{\max}]} \pcfn(uv, \tau))$.
