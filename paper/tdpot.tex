...
This definition is based on the assumption that traffic predictions are conservative estimates and that live traffic will only be slower due to accidents and other traffic incidents.
...
During the preprocessing, a total order $v_1 \prec \dots \prec v_n$ among all vertices $v_i \in V$ by ``importance'' is determined heuristically, where more important vertices should lie on more shortest paths.
...
Shortcut edges $uv$ allow to ``skip over'' paths $(u,\dots,x_i,\dots,v)$ where $x_i \prec u \land x_i \prec v$.
Therefore, $w^+(uv)$ gets the length of the shortest such path.
We sometime refer to $G^+$ split into an upward graph $\gchu = (V, \echu)$ which contains only edges $uv$ where $v \succ u$ and a downward graph $G^{\downarrow} = (V, E^{\downarrow})$ vice versa.
...
Therefore, by significantly improving query times over CCH-Potentials, our approach is the first to achieve almost real-time queries across all distances in a combined live and predicted traffic setting. % TODO reword
...
This clearly shows the superiority of IMP over MMP for long-range queries. % TODO reword
...
% function
\draw[thick] (0,1.5) -- (0.5,1.5) -- (1, 2.2);
\draw[thick,dotted] (1,2.2) -- (1.5,2.2) -- (2,3) -- (3,2.5) -- (3.7, 2) -- (4,2);
\draw[thick] (1.84375,2.75) -- (2,3) -- (2.5,2.75);
\draw[thick] (4,2) -- (4.2,2) -- (5,2.5) -- (5.5,2.5) -- (6,2.1) -- (6.4,2.1) -- (7.5,1.5) node[anchor=west] {$\pred(e)$};

\draw[thick,red] (1.84375,2.75) -- (1,2.75) node[anchor=east,red] {$l(e)$};
\draw[thick,red,dotted] (1.84375,2.75) -- (2.5,2.75);
\draw[thick,red] (2.5,2.75) -- (3.25,2.75);
\draw[thick,red,dotted] (3.25,2.75) -- (4,2.75);

\draw[thick,dotted,blue] (3.25,2.75) -- (2.5,3.5) node[anchor=south,blue, align=center] {$\pred(e, \tau^{\operatorname{end}}(e))$\\$+ \tau^{\operatorname{end}}(e) - \tau$};
\draw[thick,blue] (3.25,2.75) -- (4,2);
\end{tikzpicture}
\caption{
...
Dotted lines are only visualizations.
}
...
In~\cite{strasser_et_al:LIPIcs.SEA.2021.6}, a similar model to ours for combined traffic was proposed.
However, our model slightly defers in two ways.
