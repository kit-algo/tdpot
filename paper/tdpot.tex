...
We refer to the travel time of this path as $\dist_c(s,t,\tdep)$.
...
Dijkstra's algorithm computes $\dist_w(s,t,\tdep)$ by exploring vertices ordered by increasing distance from $s$ until $t$ is reached.
...
\begin{itemize}
  \item Feasibility: $w(uv, \tau) + \pi_t(v, \tau + w(uv, \tau)) - \pi_t(u, \tau) \geq 0$ for all edges $uv \in E$ and times $\tau$
