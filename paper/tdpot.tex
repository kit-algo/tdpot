...
For CCH, the construction of the augmented graph is split in two stages.
In the first stage, the topology of the augmented graph is constructed without considering any weight functions.
It therefore is valid for \emph{all} weight functions.
In the second stage, the \emph{customization}, the weights of the augmented graph are computed for a given weight function on the input network.
It has three steps.
...
We do not keep any additional intervals between 22:00 and 6:00 as during this time most edges will have their freeflow travel time.
...
Additionally, we compare against the time-independent CCH-Potential which is the only other algorithm we are aware of to support our problem model.
...
IMP is significantly more expensive because of the time-dependent CATCHUp preprocessing it includes.
This is especially pronounced on OSM Ger where the time-dependent travel time functions fluctuate strongly.
Still, running preprocessing algorithms on a daily basis is easily possible.
Real-time traffic updates are possible within significantly less than a minute for both our time-dependent potentials.
...
We can reduce the number of weight functions to 16 (a memory usage reduction of about a factor of 6) before the slowdowns become even noticeable in the mean running time.
...
With 32 functions, the absolute memory consumption goes down to less than 20\,GB which is quite manageable.
