...
To enable fast queries, we generalize A* potentials to time dependency, i.e.\ the estimate of the distance from a vertex to the target also depends on the time of day when the vertex is visited.
...
We achieve a speedup of at least two orders of magnitude over Dijkstra's algorithm and up to one order of magnitude over competing state-of-the-art approaches.
\end{abstract}
...
This three-phase setup has proven to be instrumental to support live traffic scenarios in practical applications~\cite{bingblog}. % TODO
...
\begin{itemize}
  ...
  \item Lower bound: $\pi_t(v, \tau) \leq \dist_w(v,t,\tau)$ for every vertex $v \in V$ and time $\tau = \dist_w(s,v,\tdep)$.
        A* with a lower bound potential will have found the correct distance once the target vertex was settled.
        This also suffices for correctness.
        However, without feasibility, A* may settle vertices multiple times.
        In theory, this can lead to an exponential running time.
\end{itemize}
In Appendix~\ref{sec:appendix:correctness} we discuss these correctness properties in detail and prove that queries are answered correctly with these properties.
...
During the update phase, we compute an additional lower bound weight function for $[\tnow, \tnow + 59\,\operatorname{minutes}]$ which incorporates the current live traffic situation $\comb$ and run the basic customization for it.
...
\end{tikzpicture}
\caption{
Example of a combined travel time function $\comb(e, \tau) = \max(\pred(e, \tau), \min(\live(e), \pred(e, \tend(e)) + \tend(e) - \tau))$ with both predicted and live traffic information.
...
Similarly to the time-independent case, we can define a modified weight function $w'$ such that running A* on a graph $G$ with time-dependent travel times $w$ with a time-dependent potential $\pi_t$ is equivalent to running Dijkstra's algorithm on the same graph with modified weights $w'$.
...
Thus, we get $\hat{w}'(uv, \tau') = \hat{\pi}_t(v, \hat{w}(uv, \hat{\pi}_t(u)^{-1}(\tau')))$ where $\hat{\pi}_t(u)^{-1}$ is the inverted arrival time function, i.e.\ a \emph{departure time} function.
...
This breaks the following correctness argument, but likely not the correctness.

Shortest paths for these modified weights are the same as with the original weights.
...
\end{align*}
This formulation also shows that the time-dependent feasibility is a generalization of the classical feasibility property.
...
This is because every vertex on the shortest path must have a queue key smaller than the target and by induction all these vertexes must have been settled before the target.
