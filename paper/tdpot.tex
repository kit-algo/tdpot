...
Let $r(v)$ denote the \emph{rank} of vertex $v$ in this order.
An \emph{augmented graph} $G^+ = (V, E^+)$ with additional \emph{shortcut edges} is constructed.
We split $G^+$ into $\gchu$ and $G^{\downarrow}$ where $\gchu$ only contains arcs $uv$ where $r(u) < r(v)$ and $G^{\downarrow}$ vice versa.
The shortcuts in $G^+$ are inserted in such a way that between any two vertices $s$ and $t$ there exists a path $P$ with $w(P) = \dist_w(s,t)$ and $r(v) \geq \min(r(s), r(t)) \forall v \in P$.
Such a path can be found by running Dijkstra's algorithm from both terminals and only following edges toward more important vertices, i.e.\ from $s$ on $\gchu$ and from $t$ on $\gchd$.
For this, edges $uv \in E^+$ graph get the weight of the shortest path between their endpoints which uses only vertices of lower rank.
We denote this \emph{lower rank distance} by $\dist^{r<}_w(u,v)$.
% shortcut weight notation
...
The CATCHUp \emph{basic} customization yields for each edge in the augmented graph \emph{time-dependent} bound functions which correspond to the shortest path between the edges endpoints through lower ranked nodes.
From these functions, scalar bounds can be derived which are typically much tighter than bounds obtained by a time-independent customization on input function bounds.
CATCHUp does not support a time-dependent \emph{perfect} customization.
However, a decent number of unnecessary edges can still be identified by running the time-independent perfect customization with the scalar upper bounds.
