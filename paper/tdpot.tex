...
% TODO talk about -1 segment, visualize
% TODO inf

Our goal is given vertices $s$ and $t$ and an earliest departure $\tau^{\operatorname{dep}}$ to obtain the path $P = (s,\dots,t)$ that minimizes $\comb_P(\tau^{\operatorname{dep}})$.
We refer to the travel time of this path as $\opttt(s,t,\tau^{\operatorname{dep}})$.
...
During the third and final \emph{query} phase, many shortest path queries $(s,t,\tau^{\operatorname{dep}})$ where $\tau^{\operatorname{dep}} \geq \tau^{\operatorname{now}}$ should be answered as quickly as possible.
