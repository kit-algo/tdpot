...
We present two extensions of Lazy RPHAST which realize a time-dependent potential function and discuss how to apply them to queries in a combined live and predicted traffic setting.
...
% The shortcuts in $G^+$ are inserted in such a way that

% We split $G^+$ into $\gchu$ and $G^{\downarrow}$ where $\gchu$ only contains arcs $uv$ where $u < v$ and $G^{\downarrow}$ vice versa.
% For this, the weights $w^+(uv)$ must have the length of the shortest path the shortcut ``skips over'', i.e.\ $w^+(uv) = \dist_{[R(u,v)]}(u,v)$.
...
% We denote this \emph{lower rank distance} by $\dist^<_w(u,v)$.
% shortcut weight notation

% \subsection{Lazy RPHAST and CH-Potentials}

\begin{algorithm2e}
\KwData{$\mathtt{D}^{\downarrow}[u]$: tentative distance from $u$ to $t$ computed by Dijkstra's algorithm on the reversed $G^+$ following only edges towards more important vertices}
...
\Fn{\Dist{$u$}}{
    \If{$\mathtt{D}[u] = \bot$}{
        ...
        \For{all edges $uv \in E^+$ where $u < v$}{
            $d \leftarrow \Dist(v)$\;
...
As before, the first step is to perform the CCH preprocessing.
...
$\underline{b'^+}(uv, \tau) := \min_{\lfloor \frac{\tau}{x} \rfloor x \leq \tau' < (\lfloor \frac{\tau}{x} \rfloor + 1) x} \underline{b^+}(uv, \tau')$
where $x$ is the length of each constant segment, i.e.\ in our case 15 minutes.
This allows us to represent the time-dependent lower bound of each edge in $E^+$ with a fixed number of travel time values.

\subparagraph{Update}
We extract an upper bound weight function $\comb_{\max}$ for the entire day valid with respect to both the predicted and the live traffic.
Then both the basic and the perfect customization algorithms are used to obtain $\comb^*_{\max}$ without constructing a reduced augmented graph.
Now, we remove all CCH edges, where the upper bound $\comb^*_{\max}$ undercuts the scalar lower bound from the CATCHUp customization $b^+_{\min}$ and prepare the reduced graph $G^*$.
We use its topology for the bucket-based time-dependent lower bound functions $\underline{b'^+}$ and the scalar bounds $b^+_{\min}$ and $\comb^+_{\max}$.
The construction of $G^+$ dominates the running time of this phase.
However, since this is optional, one can again accelerate the update phase at the cost of some query performance.
...
The first one is executed on $G^*$ with the scalar bounds $b^+_{\min}$ and $\comb^+_{\max}$.
However, for this instance, we switch the directions and thus compute lower and upper bounds on the distance from the source vertex.
The second one computes lower bound distances to the target using the $\underline{b'^+}$ weight function.
To determine evaluation times, the first Lazy RPHAST is used to quickly obtain arrival intervals at arbitrary vertices.
