...
This is because every vertex on the shortest path must have a queue key less or equal than the target when discovered with optimal distance.
...
When a vertex is discovered with suboptimal distance, the queue key may be arbitrarily larger -- but never smaller because of the FIFO property.
Therefore, the inductive argument still applies and all vertices on the shortest path must have been discovered before the target is settled.
